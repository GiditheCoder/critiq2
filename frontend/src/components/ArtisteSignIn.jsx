...
const ArtisteSignIn = () => {
  ...
  useEffect(() => {
    if (isSignedIn) {
      // If already signed in, check their role and redirect
      navigate('/works', { replace: true });
    }
  }, [isSignedIn, navigate]);
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (finalAttempt.status === "complete" && finalAttempt.createdSessionId) {
        // 👉 Fetch the user object to check their role
        const { user } = finalAttempt;
        const role = user?.publicMetadata?.role || user?.unsafeMetadata?.chosenRole;

        if (role !== "artiste") {
          // 🔥 IMPORTANT: Clean up the session before rejecting
          try {
            await signOut(); // Sign out the user
            console.log("❌ Non-artiste user signed out");
          } catch (cleanupError) {
            console.error("Cleanup error:", cleanupError);
          }
          
          alert("This account is not an artiste account. Please use an artiste account or create one.");
        ...
        // ✅ User is an artiste, proceed with sign-in
        await setActive({ session: finalAttempt.createdSessionId });
        console.log("✅ Artiste signed in successfully");
        navigate("/works", { replace: true });
      } else {
        alert("Sign in failed - incomplete authentication");
      }
    } catch (err) {
